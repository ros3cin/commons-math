...
public class WelzlEncloser<S extends Space, P extends Point<S>> implements Encloser<S, P> {
    ...
    private EnclosingBall<S, P> pivotingBall(final Iterable<P> points) {
        ...
        final List<P> extreme = new org.apache.commons.collections4.list.NodeCachingLinkedList<P>(first.getSpace().getDimension() + 1);
        final List<P> support = new org.eclipse.collections.impl.list.mutable.FastList<P>(first.getSpace().getDimension() + 1);
