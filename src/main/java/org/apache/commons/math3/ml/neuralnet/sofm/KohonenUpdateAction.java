...
public class KohonenUpdateAction implements UpdateAction {
    ...
    public void update(Network net, double[] features) {
        ...
        if (currentNeighbourhood > 0) {
            ...
            Collection<Neuron> neighbours = new org.eclipse.collections.impl.set.mutable.UnifiedSet<Neuron>();
            neighbours.add(best);
            // Winning neuron must be excluded from the neighbours.
            final java.util.Set<Neuron> exclude = new org.eclipse.collections.impl.set.mutable.UnifiedSet<Neuron>();
            exclude.add(best);
            int radius = 1;
            do {
